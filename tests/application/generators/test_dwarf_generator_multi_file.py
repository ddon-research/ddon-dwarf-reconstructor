...
@pytest.mark.unit
def test_multi_file_generation_groups_headers(tmp_path: Path) -> None:
    generator = _generator(tmp_path)
    infos = {"A": _class("A", "one.cpp"), "B": _class("B")}
    generator.workflow.expand_typedef_search = Mock()
    generator.workflow.build_hierarchy_with_timing = Mock(return_value=(infos, ["A", "B"]))
    ...
    generator.workflow.build_file_registry = Mock(return_value=registry)
    generator.workflow.render_file_headers = Mock(return_value={"one.h": "A header"})
    generator.workflow.render_uncategorized_header = Mock(
        return_value={"UncategorizedDefinitions.h": "B header"}
    )
