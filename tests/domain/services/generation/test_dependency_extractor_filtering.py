...
class TestDependencyExtractor:
    ...
    def test_filter_resolvable_types_excludes_forward_declarations(
        self, extractor, mock_dwarf_index
    ):
        """Forward declarations should not trigger full dependency resolution."""
        mock_die = Mock()
        mock_die.tag = "DW_TAG_class_type"
        mock_die.attributes = {
            "DW_AT_name": Mock(value=b"cpMotionSe"),
    ...
    def test_get_type_name_returns_none_for_invalid_offset(self, extractor, mock_dwarf_index):
