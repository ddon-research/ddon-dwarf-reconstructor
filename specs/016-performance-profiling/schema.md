...
| `runs` | workload, cold/warm state, status, timestamp, return code, Git, Python/platform/machine, source identity, configuration fingerprint, profiler mode, manifest path |
...
comparison key is workload, cold/warm state, source identity, Python version, platform, machine
profile, configuration fingerprint, and profiler mode. Real-asset rows are report-only; fixture
budgets are explicit code/test thresholds.
