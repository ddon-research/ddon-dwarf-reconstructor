...
| `application/` | Core, domain, and the named legacy generator façade | Infrastructure modules or concrete external adapters |
...
durable source-hash adapter. Direct construction remains compatible through the
streaming source-hash fallback and the named legacy ELF façade seam.
...
- Thin compatibility re-exports delegate to one canonical core policy.
