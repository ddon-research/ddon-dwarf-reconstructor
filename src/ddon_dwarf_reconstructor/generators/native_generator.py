...
class NativeDwarfGenerator:
    ...
    def find_class(self, class_name: str) -> Optional[tuple[CompileUnit, DIE]]:
                ...
                if die.tag in ('DW_TAG_class_type', 'DW_TAG_structure_type'):
                    ...
                    if name_attr and name_attr.value == target_name:
                        ...
                        if size_attr and size_attr.value > 0:
                            logger.info(f"Found {class_name} in CU at offset 0x{cu.cu_offset:x} (size: {size_attr.value} bytes)")
                            return cu, die
                        elif die.has_children:
                            logger.info(f"Found {class_name} in CU at offset 0x{cu.cu_offset:x} (has members)")
                            return cu, die
                        else:
        ...
        if fallback_candidate:
            ...
            logger.info(f"Found {class_name} in CU at offset 0x{cu.cu_offset:x} (forward declaration)")
            return cu, die
    ...
    def calculate_packing_info(self, class_info: ClassInfo) -> Dict[str, int]:
        ...
        sorted_members = sorted([m for m in class_info.members if m.offset is not None], 
        ...
        type_sizes = {
            'bool': 1, 'char': 1, 'u8': 1, 's8': 1,
            'u16': 2, 's16': 2, 'short': 2,
            'u32': 4, 's32': 4, 'int': 4, 'float': 4,
            'u64': 8, 's64': 8, 'long': 8, 'double': 8,
            'void*': 8, 'ptr': 8  # Assume 64-bit pointers
        }
        
        for i, member in enumerate(sorted_members):
            # Estimate member size
            member_size = type_sizes.get(member.type_name.replace('*', 'ptr'), 8)  # Default to pointer size
    ...
    def find_typedef(self, typedef_name: str) -> Optional[tuple[str, str]]:
        ...
        for cu in self.dwarf_info.iter_CUs():
            cu_count += 1
            logging.debug(f"Searching CU #{cu_count} at offset 0x{cu.cu_offset:x} for typedef {typedef_name}")
            ...
            for die in cu.iter_DIEs():
                ...
                if die.tag == 'DW_TAG_typedef':
                    name_attr = die.attributes.get('DW_AT_name')
                    if name_attr and name_attr.value == target_name:
                        logging.debug(f"Found typedef {typedef_name} at DIE offset 0x{die.offset:x} in CU #{cu_count}")
        ...
        logging.debug(f"Typedef {typedef_name} not found after searching {cu_count} CUs and {die_count} DIEs")
        return None
    ...
    def collect_used_typedefs(self, class_info: ClassInfo) -> Dict[str, str]:
        ...
        for member in class_info.members:
            ...
            if result:
                ...
                logging.debug(f"Found typedef for member {member.name}: {typedef_name} -> {underlying_type}")
            ...
            if method.parameters:
                ...
                for param in method.parameters:
                    ...
                    if result:
                        typedef_name, underlying_type = result
                        used_typedefs[typedef_name] = underlying_type
                        logging.debug(f"Found typedef for parameter {param.name}: {typedef_name} -> {underlying_type}")
    ...
    def resolve_type_name(self, die: DIE, type_attr_name: str = 'DW_AT_type') -> str:
        ...
        try:
            ...
            if type_die.tag == 'DW_TAG_typedef':
                ...
                typedef_name = name_attr.value.decode('utf-8') if isinstance(name_attr.value, bytes) else str(name_attr.value)
                return typedef_name
                ...
                return type_die.tag.replace('DW_TAG_', '')
            else:
    ...
    def parse_class_info(self, cu: CompileUnit, class_die: DIE) -> ClassInfo:
        ...
        if decl_file_attr:
            ...
            if cu.get_top_DIE().get_full_path():
                try:
                    ...
                    if line_program and decl_file_attr.value < len(line_program.header.file_entry):
                        ...
                        declaration_file = file_entry.name.decode('utf-8') if hasattr(file_entry.name, 'decode') else str(file_entry.name)
                except:
                    pass
        ...
        for child in class_die.iter_children():
            if child.tag == 'DW_TAG_member':
                    ...
                    try:
                        ...
                        if type_die and type_die.tag == 'DW_TAG_union_type':
                            ...
                            if union_info:
                                ...
                                logging.debug(f"Found anonymous union in {class_name}: ({union_info.byte_size} bytes)")
                            continue
                    except Exception as e:
                ...
                if struct_info:
                    ...
                    logging.debug(f"Found nested structure in {class_name}: {struct_info['name']} ({struct_info['size']} bytes)")
                ...
                if child.offset not in processed_union_offsets:
                    ...
                    if union_info:
                        unions.append(union_info)
                        logging.debug(f"Found union in {class_name}: {union_info.name} ({union_info.byte_size} bytes)")
            elif child.tag == 'DW_TAG_array_type':
                # Parse array type with size information
                array_info = self.parse_array_type(child)
                if array_info:
                    logging.debug(f"Found array type in {class_name}: {array_info['name']} (size: {array_info['total_elements']})")
                ...
                logging.debug(f"Found nested type in {class_name}: {child.tag} (name: {child_name_str})")
            else:
                # Log warning for unhandled tags
                child_name = child.attributes.get('DW_AT_name')
                child_name_str = child_name.value.decode('utf-8') if child_name else 'unnamed'
                logging.warning(f"Unhandled DWARF tag in class {class_name}: {child.tag} (name: {child_name_str}) at offset 0x{child.offset:x}")
    ...
    def parse_nested_structure(self, struct_die: DIE) -> Optional[Dict]:
        ...
        struct_name = None
        if name_attr:
            struct_name = name_attr.value.decode('utf-8') if isinstance(name_attr.value, bytes) else str(name_attr.value)
        ...
        for child in array_die.iter_children():
            if child.tag == 'DW_TAG_subrange_type':
                ...
                if count_attr:
                    ...
                    logging.debug(f"Subrange bounds: {lower_bound} to {upper_bound}, size: {dimension_size}")
                else:
                    # Unknown size
                    dimension_size = 0
    ...
    def generate_struct_definition(self, struct_info: StructInfo) -> List[str]:
        ...
        sorted_members = sorted([m for m in struct_info.members if m.offset is not None], 
                               key=lambda m: m.offset or 0)
    ...
    def _estimate_member_size(self, type_name: str) -> int:
        ...
        type_sizes = {
            ...
            'u64': 8, 's64': 8, 'long': 8, 'double': 8, 'f64': 8,
        }
    ...
    def generate_complete_hierarchy_header(self, class_name: str) -> str:
        ...
        for class_info in all_class_infos.values():
            for member in class_info.members:
                if not member.type_name.endswith('*') and member.type_name not in ('int', 'char', 'float', 'double', 'void', 'bool'):
                    if member.type_name.startswith('const '):
                        clean_type = member.type_name[6:].strip()
                    else:
                        clean_type = member.type_name
                    if not clean_type.endswith('*') and clean_type not in ('unknown_type', 's8', 's16', 's32', 's64', 'u8', 'u16', 'u32', 'u64'):
                        ...
                        if clean_type not in hierarchy_order:
                            ...
                            if clean_type.startswith('Mt') and len(clean_type) < 20:  # Basic Mt types
        ...
        lines.extend([
            "",
            f"#endif // {class_name.upper()}_HIERARCHY_H"
        ])
    ...
    def _generate_single_class(self, class_name: str, class_info: ClassInfo) -> List[str]:
        ...
        logging.debug(f"Total methods: {len(class_info.methods)}, Non-virtual methods: {len(non_virtual_methods)}")
        for i, method in enumerate(class_info.methods):
            logging.debug(f"Method {i+1}: {method.name}, virtual={method.is_virtual}, constructor={method.is_constructor}, destructor={method.is_destructor}")
        if non_virtual_methods:
            ...
            operators = [m for m in non_virtual_methods if not m.is_constructor and not m.is_destructor and m.name.startswith('operator')]
            other_methods = [m for m in non_virtual_methods if not m.is_constructor and not m.is_destructor and not m.name.startswith('operator')]
            ...
            for method in constructors:
                ...
                if method.parameters:
                    actual_params = [p for p in method.parameters if not self._is_artificial_param(p)]
            ...
            for method in operators:
                ...
                if method.return_type and method.return_type != "void":
                    ...
                    if method.name in ('operator+=', 'operator-=', 'operator*=', 'operator/=', 'operator='):
        ...
        if class_info.members:
            ...
            for member in regular_members:
                offset_comment = f"  // offset: 0x{member.offset:x}" if member.offset is not None else ""
    ...
    def generate_header(self, class_name: str) -> str:
        ...
        if class_info.packing_info:
            ...
            if packing['total_padding'] > 0:
                lines.append(f"// - Total Padding: {packing['total_padding']} bytes")
                lines.append(f"// - Natural Size: {packing['natural_size']} vs Actual Size: {packing['actual_size']}")
        ...
        if full_hierarchy:
            ...
            lines.append(f"// - Direct Inheritance: {' -> '.join(class_info.base_classes)} -> {class_name}")
        ...
        for member in class_info.members:
            if not member.type_name.endswith('*') and member.type_name not in ('int', 'char', 'float', 'double', 'void', 'bool'):
                ...
                if not clean_type.endswith('*') and '[' not in clean_type and ']' not in clean_type and clean_type not in ('unknown_type') and clean_type not in enum_names and clean_type not in struct_names and clean_type not in union_names and clean_type not in typedef_names:
                    forward_decls.add(clean_type)
        ...
        lines.extend([
            "",
            f"class{alignment_attr} {class_name}{inheritance_part}",
            "{"
        ])
