...
class LazyIndexLookupMixin:
    ...
    def get_die_by_offset(self: LazyIndexContext, offset: int) -> DIE | None:
        """Resolve a DIE through the bounded runtime cache."""
        cached_die = cast(DIE | None, self.die_cache.get(offset))
    ...
    def _find_die_at_offset(self: LazyIndexContext, offset: int) -> DIE | None:
        ...
        try:
            if not self.dwarf_info:
                ...
                return None
            die = cast(DIE | None, self.dwarf_info.get_DIE_from_refaddr(offset))
            if die is None:
                return None
    ...
    def _scan_die_at_offset(self: LazyIndexContext, offset: int) -> DIE | None:
    ...
    def _get_cu_by_offset(self: LazyIndexContext, cu_offset: int) -> CompileUnit | None:
